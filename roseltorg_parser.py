...
"""
Roseltorg parser (44-FZ / 223-FZ) via zakupki.gov.ru RSS + optional detail enrichment.

Core idea:
- Use EIS RSS (extended search): https://zakupki.gov.ru/epz/order/extendedsearch/rss.html
- Filter notices by ETP tokens: "РОСЭЛТОРГ", "АО \"ЕЭТП\"", "ЕЭТП", "ROSELTORG.RU", "roseltorg.ru"
- Normalize key fields (price, deadlines, etc.) from RSS description
- Optional: try to enrich 44-FZ by XML print form (best-effort)

Note: RSS description keys differ between 44-FZ and 223-FZ; parser uses best-effort mapping.
"""
...
EIS_XML_44_URL = "https://zakupki.gov.ru/epz/order/notice/printForm/viewXml.html"
...
class Tender:
    ...
    bids_deadline: Optional[str] = None  # ISO8601 or raw string
...
class RoseltorgEisParser:
    def __init__(self, http: Optional[HttpClient] = None, tokens: Tuple[str, ...] = ROSELTORG_TOKENS):
    ...
    def parse(self, rss_xml: str) -> List[Tender]:
        ...
        for it in items:
            ...
            # ETP field differs between 44/223 and notice types
            etp = (
                fields.get("Наименование электронной площадки в информационно-телекоммуникационной сети «Интернет»")
                or fields.get("Наименование электронной площадки в информационно-телекоммуникационной сети «Инternet»")
    ...
    def search(
        ...
        *,
        ...
        # Handle empty query for RSS if needed, though usually RSS needs a query or parameters
        # If query is empty, we must provide some other params or default to something
        q = query if query else " " 
        
        rss_url = self.build_rss_url(q, fz44=fz44, fz223=fz223, extra_params=extra_rss_params)
        try:
            rss_xml = self.fetch(rss_url)
            tenders = self.parse(rss_xml)
            return tenders[:max_items]
        except Exception as e:
            print(f"Roseltorg parser error: {e}")
            return []
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Roseltorg parser via zakupki.gov.ru RSS")
    ...
    ap.add_argument("--out", default="roseltorg_tenders.json", help="Output file (.json or .csv)")
