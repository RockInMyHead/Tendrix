...
# "Реальное время": не добавляем искусственную задержку между сообщениями.
# Ограничения Telegram всё равно применяются на стороне API (429 + retry_after),
# это обрабатывается в _send_to_telegram().
SEND_DELAY = float(os.environ.get("TELEGRAM_SEND_DELAY", "0").strip() or "0")
WORKERS = int(os.environ.get("TELEGRAM_SEND_WORKERS", "4").strip() or "4")
...
def _parse_deadline(date_str):
    """Парсит популярные форматы даты/времени. Возвращает datetime или None.

    Поддержка:
    - DD.MM.YYYY [HH:MM]
    - YYYY-MM-DD [HH:MM[:SS]] (часто приходит как publish/deadline)
...
def _is_deadline_in_future(date_str):
    ...
    Если срок не распознан, считаем тендер "доступным" для real-time уведомлений.
    """
    ...
    try:
        ...
        with SessionLocal() as db:
            for u in db.query(User).filter(User.telegram_id.isnot(None)).all():
                ...
                # Требование: если Pro, но описание компании пустое — слать все тендеры.
                if not desc:
                    out.append((tid, label))
                    continue
...
def _ensure_worker():
    if not _worker_started[0]:
        ...
        _workers_started[0] = n
        for i in range(n):
            t = threading.Thread(target=_worker, daemon=True, name=f"tg-notify-{i+1}")
            t.start()
