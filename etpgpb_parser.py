...
"""
ETP GPB (etpgpb.ru) parser (44-FZ / 223-FZ) via zakupki.gov.ru RSS (extended search).

Pipeline:
1) Fetch EIS RSS for query (44/223).
2) Parse RSS items.
3) Keep only notices where ETP field/text matches GPB ETP tokens:
   - "ЭТП Газпромбанка", "Газпромбанк", "etpgpb.ru", "ETPGPB"
4) Normalize key fields (object name, price, deadline).
5) Export JSON/CSV.

This is designed to be stable and extendable (same DTO as previous ETP parsers).
"""
...
class EtpGpbEisParser:
    def __init__(self, http: Optional[HttpClient] = None, tokens: Tuple[str, ...] = ETPGPB_TOKENS):
    ...
    def search(
        ...
        *,
        ...
        try:
            ...
            print(f"ETP GPB parser error: {e}")
            return []
...
def main() -> int:
    ap = argparse.ArgumentParser(description="ETP GPB parser via zakupki.gov.ru RSS")
    ...
    ap.add_argument("--out", default="etpgpb_tenders.json", help="Output file (.json or .csv)")
    ...
    parser = EtpGpbEisParser(http=HttpClient(user_agent=ns.ua))
