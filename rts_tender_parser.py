...
"""
RTS-tender parser (44-FZ / 223-FZ) via zakupki.gov.ru RSS (extended search).

Pipeline:
1) Fetch EIS RSS for query (44/223).
2) Parse RSS items.
3) Keep only notices where ETP field/text matches RTS-tender tokens:
   - "РТС-тендер", "ООО «РТС-тендер»", "RTS-tender", "rts-tender.ru"
4) Normalize common fields: object name, price, deadline, etc.
5) Export JSON/CSV.

Notes:
- EIS extended search exists and is convenient for discovery by keywords.
- ETP naming in EIS is stable enough to filter RTS-tender reliably.
"""
...
class RtsTenderEisParser:
    def __init__(self, http: Optional[HttpClient] = None, tokens: Tuple[str, ...] = RTS_TENDER_TOKENS):
    ...
    def search(
        ...
        *,
        ...
        try:
            ...
            print(f"RTS-tender parser error: {e}")
            return []
...
def main() -> int:
    ap = argparse.ArgumentParser(description="RTS-tender parser via zakupki.gov.ru RSS")
    ...
    ap.add_argument("--out", default="rts_tender_tenders.json", help="Output file (.json or .csv)")
    ...
    parser = RtsTenderEisParser(http=HttpClient(user_agent=ns.ua))
