...
interface TelegramConnectDialogProps {
    ...
    /** Новый код для повторной привязки в боте (профиль → обновить код) */
    forceRegenerate?: boolean;
}

export const TelegramConnectDialog = ({ open, onOpenChange, forceRegenerate = false }: TelegramConnectDialogProps) => {
    ...
    const fetchCode = useCallback(async () => {
        ...
        try {
            ...
            const q = forceRegenerate ? '?regenerate=true' : '';
            const res = await fetch(`/api/users/me/telegram-link${q}`, {
    ...
    useEffect(() => {
        ...
        setCopied(false);
        void fetchCode();
    }, [open, forceRegenerate, fetchCode]);
