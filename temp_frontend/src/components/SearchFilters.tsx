...
import { categories, regions, procurementTypes } from '@/data/mockTenders';
...
export const SearchFilters = ({
  ...
  const updateFilter = <K extends keyof TenderFilters>(key: K, value: TenderFilters[K]) => {
    const resetPageKeys: (keyof TenderFilters)[] = ['search', 'region', 'source', 'budgetFrom', 'budgetTo', 'law44', 'law223', 'category', 'procurementType'];
    const resetPage = resetPageKeys.includes(key);
    const resetAi = key !== 'aiSearch';
    onFiltersChange({ ...filters, [key]: value, ...(resetPage ? { page: 1 } : {}), ...(resetAi ? { aiSearch: false } : {}) });
  };
  ...
  const handleAiSearch = () => {
    ...
    onFiltersChange({ ...filters, aiSearch: true, page: 1 });
  };
