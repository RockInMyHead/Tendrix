import { Bell, Heart, Send, LogOut, Sparkles, Shield, User } from 'lucide-react';
...
interface HeaderProps {
  ...
  isAdmin?: boolean;
  /** Вызывается после успешной активации Pro — чтобы дашборд сразу показал блок AI без перезагрузки */
  onProActivated?: () => void;
}

export const Header = ({ favoritesCount, onShowFavorites, showFavorites, isAdmin, onProActivated }: HeaderProps) => {
  ...
  const openTelegramBot = async () => {
    try {
      const res = await fetch('/api/public/telegram-bot');
      const data = await res.json();
      if (data?.url) {
        window.location.assign(data.url);
        return;
      }
      toast.error('Ссылка на бота не настроена на сервере');
    } catch {
      toast.error('Не удалось открыть Telegram');
    }
  };
