...
"""
ZakazRF (AGZ RT) parser (44-FZ / 223-FZ) via zakupki.gov.ru RSS (extended search).

Pipeline:
1) Fetch EIS RSS for query (44/223).
2) Parse RSS items.
3) Keep only notices where ETP field/text matches ZakazRF tokens.
...
ZAKAZRF_TOKENS = (
    ...
    "АО \"АГЗ РТ\"",
    "АГЗ РТ",
    "Агентство государственного заказа РТ",
    "Агентство государственного заказа Республики Татарстан",
)
...
class ZakazRfEisParser:
    def __init__(self, http: Optional[HttpClient] = None, tokens: Tuple[str, ...] = ZAKAZRF_TOKENS):
    ...
    def search(
        ...
        *,
        ...
        try:
            ...
            print(f"ZakazRF parser error: {e}")
            return []
...
def main() -> int:
    ap = argparse.ArgumentParser(description="ZakazRF (AGZ RT) parser via zakupki.gov.ru RSS")
    ...
    ap.add_argument("--out", default="zakazrf_tenders.json", help="Output file (.json or .csv)")
    ...
    parser = ZakazRfEisParser(http=HttpClient(user_agent=ns.ua))
