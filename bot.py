...
_link_locks: Dict[int, asyncio.Lock] = {}


def _lock_for_tg_id(tg_id: int) -> asyncio.Lock:
    lock = _link_locks.get(tg_id)
    if lock is None:
        lock = asyncio.Lock()
        _link_locks[tg_id] = lock
    return lock
...
async def command_describe_handler(message: types.Message):
    ...
    try:
        users = session.query(User).filter(User.telegram_id == message.from_user.id).all()
        if not users:
            await message.answer(
        ...
        lines = ["К этому Telegram привязано несколько аккаунтов Tendrix:", ""]
        for u in users:
            tag = "Pro" if u.is_pro else "Free"
            lines.append(f"- {u.username} ({tag})")
        lines.append("")
        lines.append("Уведомления будут приходить по каждому из привязанных аккаунтов.")
        await message.answer("\n".join(lines))
...
async def link_account(message: types.Message, code: str):
    ...
    async with _lock_for_tg_id(tg_id):
        session = SessionLocal()
        try:
            ...
            # Операции привязки сериализуем по tg_id, чтобы не ловить гонки при серии апдейтов подряд.
            for attempt in (1, 2):
                try:
                    ...
                    session.rollback()
                    logger.error(f"IntegrityError linking account (attempt {attempt}): {e}")
                    if attempt == 2:
                        raise
