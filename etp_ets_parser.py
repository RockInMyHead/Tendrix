...
"""
ETP-ETS (National electronic platform / ETP MMBV) parser via zakupki.gov.ru RSS (extended search).

Pipeline:
1) Fetch EIS RSS for query (44/223).
2) Parse RSS items.
3) Filter items by ETP tokens:
   - "etp-ets.ru", "223.etp-ets.ru"
   - "Национальная электронная площадка"
   - "ЭТП ММВБ" / "ММВБ"
   - "Электронная торговая площадка «Фабрикант»"
   - Operator naming variants (АО «Электронные торговые системы», etc.)
4) Normalize key fields: object name, price, deadline.
5) Export JSON/CSV.

This avoids fragile direct scraping of ETP UI.
"""
...
class EtpEtsEisParser:
    def __init__(self, http: Optional[HttpClient] = None, tokens: Tuple[str, ...] = ETP_ETS_TOKENS):
    ...
    def search(
        ...
        *,
        ...
        try:
            ...
            print(f"ETP-ETS parser error: {e}")
            return []
...
def main() -> int:
    ap = argparse.ArgumentParser(description="ETP-ETS (NЭП/ЭТП ММВБ) parser via zakupki.gov.ru RSS")
    ap.add_argument("--query", required=True, help="Search query (e.g. 'поставка оборудования')")
    ap.add_argument("--max", type=int, default=100, help="Max items (default: 100)")
    ap.add_argument("--out", default="etp_ets_tenders.json", help="Output file (.json or .csv)")
    ...
    parser = EtpEtsEisParser(http=HttpClient(user_agent=ns.ua))
