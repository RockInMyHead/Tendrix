...
"""
Lot-Online (RAD) parser (44-FZ / 223-FZ) via zakupki.gov.ru RSS (extended search).

Pipeline:
1) Fetch EIS RSS for query (44/223).
2) Parse RSS items.
3) Keep only notices where ETP field/text matches Lot-Online tokens.
...
class LotOnlineEisParser:
    def __init__(self, http: Optional[HttpClient] = None, tokens: Tuple[str, ...] = LOT_ONLINE_TOKENS):
    ...
    def search(
        ...
        *,
        ...
        try:
            ...
            print(f"Lot-Online parser error: {e}")
            return []
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Lot-Online (RAD) parser via zakupki.gov.ru RSS")
    ...
    ap.add_argument("--out", default="lot_online_tenders.json", help="Output file (.json or .csv)")
    ...
    parser = LotOnlineEisParser(http=HttpClient(user_agent=ns.ua))
